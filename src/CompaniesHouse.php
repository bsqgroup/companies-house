...
/*
 * This file is part of the GovTalk\CompaniesHouse package
 *
 * For the full copyright and license information, please see the LICENSE
 * file that was distributed with this source code.
 */

namespace GovTalk\CompaniesHouse;
 ...
 * GovTalk class to build and parse Companies House submissions.
 *
 * @author Kieran White
 * @copyright 2020, BSQ Group
 */
...
class CompaniesHouse extends GovTalk {
	...
	public function __construct(
        ...
        $this->setProductUri($routeURI);
        $this->setProductName($softwareName);
        $this->setProductVersion($softwareVersion);
				...
				case 'DIS': case 'LLP': case 'CUR': case 'EUR':
                    ...
                    if ($this->sendMessage() && ($this->responseHasErrors() === false)) {
						...
						foreach ($officerSearchBody->OfficerSearchItem AS $officerDetails) {
							$thisOfficerDetails = array(
                                'id' => (string) $officerDetails->PersonID,
		...
		if (preg_match('/[A-Z0-9]{8,8}/', $companyNumber)) {
			...
			if ($this->sendMessage() && ($this->responseHasErrors() === false)) {
				...
				$companyDetails = array(
                    'name' => (string) $companyDetailsBody->CompanyName,
				...
				if (isset($companyDetailsBody->Accounts)) {
					$companyDetails['accounts'] = array(
                        'overdue' => (string) $companyDetailsBody->Accounts->Overdue,
				...
				if (isset($companyDetailsBody->Returns)) {
					$companyDetails['returns'] = array(
                        'overdue' => (string) $companyDetailsBody->Returns->Overdue,
				...
				if (isset($companyDetailsBody->Mortgages)) {
					$companyDetails['mortgage'] = array(
                        'register' => (string) $companyDetailsBody->Mortgages->MortgageInd,
		...
		if (($personId != '') && (($reference != '') && (strlen($reference) < 25))) {
            ...
            if ($this->sendMessage() && ($this->responseHasErrors() === false)) {
				...
				$officerDetails = array(
                    'title' => (string) $officerDetailsBody->Person->Title,
				...
				if (isset($officerDetailsBody->OfficerAppt)) {
					...
					foreach ($officerDetailsBody->OfficerAppt AS $appointment) {
						...
						if (!array_key_exists($arrayKey, $officerDetails['company'])) {
							$officerDetails['company'][$arrayKey] = array(
                                'name' => (string) $appointment->CompanyName,
                                'number' => (string) $appointment->CompanyNumber,
                                'status' => (string) $appointment->CompanyStatus
                            );
						}
						$thisAppointment = array(
                            'type' => (string) $appointment->AppointmentType,
				...
				if (isset($officerDetailsBody->OfficerDisq)) {
					foreach ($officerDetailsBody->OfficerDisq AS $disqualifcation) {
						$thisDisqualifcation = array(
                            'reason' => (string) $disqualifcation->DisqReason,
						...
						if (isset($disqualifcation->Exemption)) {
							foreach ($disqualifcation->Exemption AS $exemption) {
								$thisDisqualifcation['exemption'][] = array(
                                    'name' => $exemption->CompanyName,
            ...
            if ($this->sendMessage() && ($this->responseHasErrors() === false)) {
				...
				if (isset($filingHistoryBody->FHistItem)) {
					...
					foreach ($filingHistoryBody->FHistItem AS $historyItem) {
						$thisHistoryItem = array(
                            'date' => strtotime((string) $historyItem->DocumentDate),
	...
	public function documentInfo($companyNumber, $companyName, $imageKey) {
		if (preg_match('/[A-Z0-9]{8,8}/', $companyNumber) && (($companyName != '') && (strlen($companyName) < 161))) {
            ...
            if ($this->sendMessage() && ($this->responseHasErrors() === false)) {
				...
				$documentInfo = array(
                    'type' => (string) $documentInfoBody->FormType,
	...
	private function _parseCompanySearchResult($companySearchBody) {
		if (is_object($companySearchBody) && is_a($companySearchBody, 'SimpleXMLElement')) {
			...
			foreach ($companySearchBody->CoSearchItem AS $possibleCompany) {
				$thisCompanyDetails = array(
                    'name' => (string) $possibleCompany->CompanyName,
